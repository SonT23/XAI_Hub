...
> 🎓 **NCKH** / [Toán](../toan/index.md) / **Đại số tuyến tính**
...
> Đây cũng là lý do PCA hay được so sánh với **Autoencoder**: cùng mục tiêu giảm chiều, nhưng PCA chỉ làm được **tuyến tính**, còn Autoencoder làm được **phi tuyến**.
...
> Cosine similarity trong **Latent Space (Không gian tiềm ẩn)**, cách CLIP ghép ảnh với chữ, cách Attention tính $QK^\top$, cách vector database tìm kết quả gần nhất — **tất cả đều chỉ là tích vô hướng**.
...
> Học sâu không phát minh ra phép toán mới. Nó chỉ **xếp chồng thật nhiều phép biến đổi tuyến tính, xen giữa là các hàm phi tuyến**, rồi dùng đạo hàm để dò tìm bộ trọng số tốt. Phần "đạo hàm để dò tìm" chính là nội dung của [Toán](../toan/index.md) → Giải tích.
